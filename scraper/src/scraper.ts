...
async function parseMensa(url: string): Promise<Day[]> {
  try {
    const response = await axios(url);
    const html = response.data;
    const $ = cheerio.load(html);
    const dayList = $("div.essenliste");
    const days: Day[] = [];
    dayList.each(function () {
      const mealList = $(this).find(
        'div[data-role="content"] > ul > li.conditional'
      );
      const meals: Meal[] = [];
      mealList.each(function () {
        ...
        const refArray = parseJSStyleString(references);
...
function toJson(map: Map<string, Day[]>) {
...
async function main() {
  /*const mensen: Mensa[] = await parseMensenList(
    "https://muenster.my-mensa.de/index.php?v=5348942&lang=en"
  );
  console.log(mensen);*/
